...
class MainApp extends PolymerElement {
  ...
  MainApp.created() : super.created() {
    ...
    if (document.getElementById('elections') == null) return; // Error message, broken HTML.
    for (TableRowElement tableRow in document.getElementById('elections').rows) {
    ...
    sortElectionsByDate();
    document.querySelector('#container').appendHtml('<div><a id="chosenelectionlink"><h1 id="chosenelection"></h1></a></div>');
    document.querySelector('#container').appendHtml("<div class='spaced'><a>News</a><a>Parliament</a> <a>Electoral System</a> <a>Economy</a></div>");
    document.querySelector('#container').appendHtml('<div class="popup"></div>');
    ...
    tsv("data/elections.tsv", callback: (final List<Map<String, Object>> data) {
      data.forEach((d) {
        ...
        if (d['data_html'] == '' || d['data_html'] == null) {
          document.querySelector('.popup').insertAdjacentHtml('afterbegin', electionBox + '<div id="wikiimport"><div id="polldata" style="text-align: center; padding: 4%; border: 1px black solid; margin-top: 4%; margin: 2%;">No data could be provided by Wikipedia.</div></div><iframe seamless class="dataiframe" style="display: none; width: 96%; height: 400px; overflow: hidden;" src="https://en.wikipedia.org/w/index.php?title=Opinion_polling_for_the_next_United_Kingdom_general_election&printable=yes"></iframe>' + '</div>');
        } else {
          document.querySelector('.popup').insertAdjacentHtml('afterbegin', electionBox + d['data_html'] + '</div>');
        }
      });
      document.getElementById('elections').children.first.children[2].click();
    });
  }
  ...
  sortElectionsByDate() {
    ...
    for (var key in datedRowOrder.keys.toList()..sort()) {
      TableRowElement row = document.getElementById('elections').rows[datedRowOrder[key]];
      print(row.outerHtml);
      newElectionsTable += '<tr id="' + row.id + '">' + row.innerHtml + '</tr>';
    }
    newElectionsTable += '</tbody>';
    document.getElementById('elections').setInnerHtml(newElectionsTable, validator: all);
    document.getElementById('elections').rows.forEach((TableRowElement row) => row.onClick.listen((e) => countryClicked(e)));
  }
